...
======================
LA Crime Data — Decision Tree (single CART classifier)
...
architecture defined in src/config.py:
    ...
    * pulls hyperparameters           -> cfg.MODEL_PARAMS["decision_tree"]
    * writes all artifacts            -> outputs/decision_tree_output/
    * upserts its metrics             -> outputs/model_comparison_summary.csv
                                         (keyed by model_key="decision_tree")
...
Unlike XGBoost, a single sklearn DecisionTreeClassifier handles string targets
natively, so there is no LabelEncoder here — we evaluate on the raw string class
labels (mirroring model_logistic_regression.py). Boosting-specific artifacts
(training curve, SHAP) are not applicable and are intentionally omitted; the
decision-tree-specific extra is a plot of the fitted tree's top levels.

Usage:
    python models/model_decision_tree.py

Produces (in outputs/decision_tree_output/):
    ...
    tree_structure.png                <- decision-tree-specific extra (top 3 levels)
    grid_search_results.csv
    model_comparison_summary.csv      <- this model's rows (also upserted to the shared file)
    model_decision_tree.joblib        <- serialized tuned pipeline
"""
...
# Approved feature set — IDENTICAL to model_xgboost.py / model_logistic_regression.py
# so the models are compared on the same inputs. weapon_desc / status_desc are
# excluded on purpose: they are downstream consequences of the crime category and
# would leak the target. Any column missing from the features file (e.g. disabled
# via config.FEATURE_CATALOGUE) is skipped so the model still runs.
...
def build_preprocessor(work: pd.DataFrame):
    """
    A decision tree is scale-invariant (splits are threshold-based), so — like
    XGBoost and unlike logistic regression — it needs NO feature scaling. We still:
...
def build_baseline_model(preprocessor):
    """
    Baseline DecisionTreeClassifier with the sensible defaults from
    config.MODEL_PARAMS.

    Why a single decision tree for this task:
      - It's the interpretable, non-linear baseline: a flowchart of threshold
        splits that captures feature interactions a linear model cannot, while
        staying easy to read (see tree_structure.png).
      - It's the conceptual building block for the random-forest and XGBoost
        ensembles, so it anchors the comparison from below.
      - Defaults grow an unpruned tree (max_depth=None); tuning then prunes it.
    """
    base = dict(PARAMS["baseline"])   # copy so we can safely augment
    model = DecisionTreeClassifier(**base, random_state=RANDOM_SEED)
...
def tune_model(preprocessor, X_train, y_train):
    ...
    The grid includes class_weight=[None, 'balanced'] so tuning can opt into
    rare-class weighting if it improves macro-F1.
    """
    print(f"\n{'='*70}")
...
# =============================================================================
# 7.  VISUALS  (all saved into outputs/decision_tree_output/)
# =============================================================================
...
def plot_roc_curves(y_test, y_proba, class_labels, filename: str):
    ...
    ax.set_title("ROC Curves — One-vs-Rest per Category (Tuned Decision Tree)",
...
def plot_feature_importance(pipeline, top_n: int = 20, filename: str = "feature_importance.png"):
    print(f"\n  [PLOT] Feature importance (impurity/Gini-based, from the tuned tree)")
    try:
...
def plot_per_class_metrics(y_test, y_pred, class_labels, filename: str):
    ...
    ax.set_title("Per-Class Precision / Recall / F1 (Tuned Decision Tree)",
                 fontsize=13, fontweight="bold", color=DARK_BG)
    ax.legend()
    plt.tight_layout()
    _save(fig, filename)
...
def plot_baseline_vs_tuned(baseline, tuned, filename: str):
    ...
    ax.set_ylim(0, 1); ax.set_ylabel("Score")
    ax.set_title("Baseline vs Tuned — Decision Tree", fontsize=13, fontweight="bold", color=DARK_BG)
...
def plot_tree_structure(pipeline, class_labels, max_depth: int = 3,
                        filename: str = "tree_structure.png"):
    """
    Decision-tree-specific extra (the template's SHAP-equivalent): draw the tuned
    tree's top `max_depth` levels with sklearn.tree.plot_tree. Capping the depth
    keeps it legible — the full tuned tree can be far deeper. Shows the actual
    threshold splits, which feature each node uses, and the majority class /
    colour at every node, which is exactly the interpretability selling point of
    a single decision tree.
    """
    print(f"\n  [PLOT] Tree structure (top {max_depth} levels of the tuned tree)")
    clf = pipeline.named_steps["classifier"]
    try:
        feat_names = _clean_feature_names(
    ...
    plot_tree(
        clf, max_depth=max_depth, feature_names=feat_names,
        class_names=list(class_labels), filled=True, rounded=True,
        impurity=False, proportion=True, fontsize=8, ax=ax,
    )
    ax.set_title(f"Decision Tree — Top {max_depth} Levels (Tuned)",
...
def write_comparison(baseline, tuned, best_params):
    ...
    comparison = pd.DataFrame([
        ...
        {"model": f"Decision Tree (Tuned: {best_params})",
...
def main():
    print(f"\n{'#'*70}")
    print("  DECISION TREE — LA Crime Category Prediction")
    ...
    # Sklearn trees handle string targets natively — no LabelEncoder needed.
    ...
    plot_confusion_matrix(baseline_results["y_test"], baseline_results["y_pred"],
                          class_labels, "Confusion Matrix — Baseline Decision Tree",
                          "confusion_matrix_baseline.png")
    ...
    plot_confusion_matrix(tuned_results["y_test"], tuned_results["y_pred"],
                          class_labels, "Confusion Matrix — Tuned Decision Tree",
                          "confusion_matrix_tuned.png")
    ...
    # ---- Decision-tree-specific extra: the fitted tree's top levels ----
    plot_tree_structure(tuned_pipeline, class_labels)

    # ---- Comparison summary ----
    write_comparison(baseline_results, tuned_results, best_params)

    # ---- Persist the tuned model ----
    model_path = OUT_DIR / "model_decision_tree.joblib"
