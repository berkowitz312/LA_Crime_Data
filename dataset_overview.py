...
data_cleaning.py first (it creates the cleaned_data/ folder), then run:

    python dataset_overview.py

Produces:
    eda_outputs/*.png            <- static charts
    eda_outputs/heatmap.html     <- interactive, filterable crime heat-map
...
def find_cleaned_file() -> Path:
    ...
    if not candidates:
        # fall back to script folder itself, in case the user moved the file
        candidates = sorted(SCRIPT_DIR.glob("*cleaned*.csv"))
    if not candidates:
        print(f"\n  [ERROR] Could not find a cleaned CSV file.")
        print(f"          Looked in: {CLEANED_DIR}")
        print(f"          Run data_cleaning.py first — it produces "
              f"cleaned_data/la_crime_cleaned.csv")
        sys.exit(1)
    if len(candidates) > 1:
